/**
 * $Id: mxDefaultTextShape.java,v 1.8 2010-07-19 10:59:35 david Exp $
			...
			mxCellState state, Map<String, Object> style)
	{
		...
		if (g.getClipBounds() == null || g.getClipBounds().intersects(rect))
		{
					...
					"nowrap").equals("wrap"))
			{
				...
				g.setFont(mxUtils.getFont(style, 1));
				FontMetrics fm2 = g.getFontMetrics();
				// The width passed into word wrap suffers from double
				// representation precision. The int width has already been * scale,
				// divide again by scale by create a double slightly under the
				// original width.
				lines = mxUtils.wordWrap(text, fm2,
						(int) ((state.getLabelBounds().getWidth() / scale) + 0.5)
								- mxConstants.LABEL_INSET * 2);
