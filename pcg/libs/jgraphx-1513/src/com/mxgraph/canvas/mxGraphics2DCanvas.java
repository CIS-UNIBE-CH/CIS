/**
 * $Id: mxGraphics2DCanvas.java,v 1.189 2011-01-05 11:20:51 gaudenz Exp $
		...
		if (imageUrl != null)
		{
			...
			if (img != null)
			{
				...
				if (preserveAspect)
				{
					...
					int x0 = (int) (bounds.width - iw * s) / 2;
					int y0 = (int) (bounds.height - ih * s) / 2;

					scaledImage = img.getScaledInstance((int) (iw * s),
							(int) (ih * s), IMAGE_SCALING);
					dx = x0 + bounds.x;
					dy = y0 + bounds.y;
					w = (int) (iw * s);
					h = (int) (ih * s);
				...
				{
					scaledImage = img.getScaledInstance(bounds.width,
							bounds.height, IMAGE_SCALING);
					dx = bounds.x;
					dy = bounds.y;
					w = bounds.width;
					h = bounds.height;
				...
				if (scaledImage != null)
				{
					...
					g.drawImage(scaledImage, dx, dy, dx + w, dy + h, sx, sy, r,
							b, null);
				}
			}
		}
